...
def _build_embedding_text(topic, summary, opinion):
    """Собирает текст для эмбеддинга. Вынесено в хелпер, чтобы не дублировать."""
    parts = [topic]
    if summary:
    ...
    if pending_reflection:
        ...
        if old_opinion == new_opinion:
            ...
            text = _build_embedding_text(
                pending_reflection["topic"],
                pending_reflection["summary"],
